...
@app.get('/mserver_performance/all_data')
@app.get('/mserver_performance/all_data/')
def get_mserver_perfcheck_get_data(db):
    return '''<form method="POST" action="/mserver_performance/get_data">
                <select name="data">
...
@app.post('/mserver_performance/all_data')
@app.post('/mserver_performance/all_data/')
def post_mserver_perfcheck_get_data(db): 
    data = bottle.request.forms.get('data')
    how = bottle.request.forms.get('how')
#table name
    f = data + ': ' + how + ', '
    c = db.execute('SELECT DISTINCT fqdn_to FROM mserver_performance WHERE success IS 1 AND fqdn_from LIKE \'bismark.gt.%\' ORDER BY fqdn_to ASC')
    fqdnlist = c.fetchall()
#x-axis lables
    for server in fqdnlist:
        f += server[0] + ', '
    f += '\n'
    c = db.execute('SELECT * FROM mserver_performance WHERE success IS 1 AND fqdn_from LIKE \'bismark.gt.%\' ORDER BY fqdn_from DESC, fqdn_to ASC')
    store = c.fetchall()
    x = 0
    innerlist = []
    while x < len(store): 
	tempfrom = store[x][1]
	f += tempfrom + ', '
	while x < len(store) and store[x][1] == tempfrom: 
	    tempto = store[x][2]
	    while x < len(store) and store[x][2] == tempto: 
		if data == 'throughput': 
		    innerlist.append(store[x][4])
		elif data == 'jitter': 
		    innerlist.append(store[x][5])
		elif data == 'loss': 
		    innerlist.append(store[x][6])
		elif data == 'latency': 
		    innerlist.append(store[x][7])
		x = x + 1
	    if how == 'max': 
		f += str(max(innerlist)) + ', '
	    elif how == 'min': 
		...
		f += str(sum(innerlist) / len(innerlist)) + ', '
	    innerlist = []
	f += '\n'
    return f

@app.get('/mserver_performance/host_data')
@app.get('/mserver_performance/host_data/')
def get_mserver_host_data(db): 
    return '''<form method="POST" action="/mserver_performance/host_data">
                <select name="data">
                        <option value="throughput">Throughput</option>
                        <option value="jitter">Jitter</option>
                        <option value="loss">Loss</option>
                ...
                <select name="direction">
                        <option value="to">To Host</option>
                        <option value="from">From Host</option>
                </select>
		<label for="host">Hostname to see: <input type="text" name="host" /> </label>
		<label for="days">Days back: <input type="text" name="days" /> </label>
                <input type="submit" value="Submit" />
              </form>'''

@app.post('/mserver_performance/host_data')
@app.post('/mserver_performance/host_data/')
def get_mserver_host_data(db): 
    data = bottle.request.forms.get('data')
    direction = bottle.request.forms.get('direction')
    host = bottle.request.forms.get('host')
    timeback = bottle.request.forms.get('timeback')

    f = direction + ': ' + host + ', '
    c = db.execute('SELECT ' + data + ', utcdate FROM mserver_performance WHERE success IS 1 AND ' + ('fqdn_to' if direction == 'to' else 'fqdn_from') + ' IS \"' + host + '\" ORDER BY utcdate DESC')
    store = c.fetchall()
    for point in store: 
	if ((datetime.datetime.utcnow() - datetime.datetime.strptime(point[1], "%Y-%m-%d %H:%M:%S")).days) < int(timeback):
	    f += str(point[1]) + ', '
    f += '\n' + data + ', '
